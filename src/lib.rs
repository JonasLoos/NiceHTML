...
use web_sys::{window, Document, console, Element, Node};
...
pub fn render(script: &str) -> Result<(), JsValue> {
    ...
    for line in lines {
        ...
        while indent <= current_indent {
            let (previous_top, var_name) = parent_stack.pop();
            current_indent -= 1;
            if var_name.empty() {
        ...
        if raw_line.ends_with("=") {
            ...
            let mut arg_places: Vec<Vec<Element>> = vec![];
            for arg_name in args {
                arg_places.push((arg_name, vec![]));
            }
            variables.insert(name, arg_places);
            var_elem = document.create_element("div")?;
        ...
        // normal tag
            ...
            for cap in re.captures_iter(attributes) {
                let name = &cap["name"];
                let value = &cap["value"];
        ...
        // variable
        } else if raw_line.starts_with("$") && is_valid_identifier(&raw_line.split("(").next()[1..]) {
            let name = &raw_line.split("(").next().unwrap()[1..];
            if variables.contains_key(name) {
                // make sure there are no references to variables that are still being defined:
                for (_, x) in parent_stack {
                    if x == name {
                        return Err("Syntax error: variable is still being defined".into());
                    }
                }
                let (var, arg_places) = variables.get(name);
                // function call
                if arg_places.len() > 0 {
                    if raw_line.split("(").count() != 2 {
                        return Err("Syntax Error: invalid function call")
                    }
                    // get given args
                    let given_args_raw = raw_line.split("(").nth(1).unwrap().split(")").next().unwrap().split(",").collect();
                    ...
                    for arg_raw in given_args_raw {
                        ...
                        if arg.starts_with("\"") && arg.ends_with("\"") {
                            ...
                            let (arg_var, arg_var_arg_places) = variables.get(name);
                            if !arg_var_arg_places.empty() {
                                return Err("Syntax Error: Function calls inside other function calls are not allowed");
                            }
